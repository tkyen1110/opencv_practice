...
RUN apt-get update && \
    apt-get install -y apt-utils build-essential software-properties-common cmake && \
    apt-get install -y sudo vim wget zip unzip curl git && \
    apt-get install -y net-tools iputils-ping

# Install apt packages
RUN apt-get install -y libopencv-dev libgtest-dev libboost-all-dev libusb-1.0-0-dev libeigen3-dev && \
    apt-get install -y libglew-dev libglfw3-dev libcanberra-gtk-module ffmpeg && \
    apt-get install -y libnvidia-gl-515 && \
...
# Install python3.7
RUN apt-get install -y python3-pip python3-tk python3-distutils && \
...
RUN python3 -m pip install --upgrade pip && \
    python3 -m pip install "opencv-python>=4.5.5.64" "sk-video==1.1.10" "fire==0.4.0" "numpy<=1.21" && \
    python3 -m pip install pandas scipy h5py profilehooks pytest && \
